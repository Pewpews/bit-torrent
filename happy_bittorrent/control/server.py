...
class ControlServer:
    ...
    HANDSHAKE_MESSAGE = b'happy-bittorrent:ControlServer\n'
    ...
    async def _accept(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            ...
            logger.exception('%s disconnected because of %r', addr_repr, e)
        finally:
            writer.close()
    ...
    async def start(self):
        for port in ControlServer.PORT_RANGE:
            try:
                ...
                logger.exception('exception on starting server on port %s', port)
            else:
